...
class additional_smtp extends rcube_plugin {
    ...
    const PLUGIN_INFO = array(
        'name' => 'additional_smtp',
        'vendor' => 'Gene Hawkins',
        'version' => self::PLUGIN_VERSION,
        'license' => 'GPL-3.0',
        'uri' => 'https://github.com/texxasrulez/additional_smtp',
    );
    ...
    function message_compose_body($args) {
        ...
        if ($rcmail->user->data['username'] != $_SESSION['username']) {
            ...
            $this->debug_log('message_compose_body', array(
                'session_username' => $_SESSION['username'] ?? null,
                'account_username' => $rcmail->user->data['username'] ?? null,
                'smtp_overwrite' => $this->smtp_overwrite,
            ));
    ...
    function message_outgoing_headers($args) {
        ...
        if ($smtp_user != '%u' || strtolower($sender[1]) != strtolower($rc_uname[1])) {
            ...
            $this->debug_log('message_outgoing_headers overwrite', array(
                'x_sender' => $args['headers']['X-Sender'] ?? null,
                'account_username' => $this->rcmail->user->data['username'] ?? null,
                'configured_smtp_user' => $smtp_user,
            ));
        }
        if (!$this->rcmail->config->get('smtp_server')) {
            $sql_arr = $this->smtp_connect(array());
            $this->debug_log('message_outgoing_headers smtp_connect result', $this->debug_smtp_context($sql_arr));
            if (count($sql_arr > 0)) {
    ...
    function smtp_connect($args) {
        ...
        $this->debug_log('smtp_connect begin', array(
            'smtp_overwrite' => $this->smtp_overwrite,
            'sender' => $this->sender,
            'account_username' => $rcmail->user->data['username'] ?? null,
            'session_username' => $_SESSION['username'] ?? null,
            'input' => $this->debug_smtp_context($args),
        ));
        ...
        if ($this->smtp_overwrite) {
            ...
            if ($instance = ($smtp_int[strtolower($sender[1])] ?? null)) {
                ...
                $source = 'internal:' . strtolower($sender[1]);
                $this->debug_log('smtp_connect matched internal', $this->debug_smtp_context($args, array(
                    'source' => $source,
                    'sender_domain' => strtolower($sender[1]),
                )));
            } else {
                ...
                foreach($argsv as $fetch) {
                    ...
                    if (is_array($sql_arr) && $sql_arr['enabled']) {
                        ...
                        $rcmail->config->set('no_save_sent_messages', $sql_arr['nosavesent'] ? true : false);
                        $source = 'identity-db:' . $fetch['identity_id'];
                        $this->debug_log('smtp_connect matched identity', $this->debug_smtp_context($args, array(
                            'source' => $source,
                            'identity_id' => $fetch['identity_id'],
                            'sender' => $this->sender,
                            'password_decrypted' => $decrypt !== false,
                        )));
                        break;
                    }
                }
        ...
        if (strtolower($rcmail->user->data['username']) != strtolower($_SESSION['username'])) {
            if (($args['smtp_user'] ?? null) == '%u') {
                $args['smtp_user'] = $rcmail->user->data['username'];
                $this->debug_log('smtp_connect resolved smtp_user placeholder', array(
                    'source' => $source,
                    'resolved_smtp_user' => $args['smtp_user'],
                ));
            }
            if (($args['smtp_pass'] ?? null) == '%p') {
                ...
                $this->debug_log('smtp_connect resolved smtp_pass placeholder', array(
                    'source' => $source,
                    'resolved_from' => isset($_SESSION['default_account_password']) && $_SESSION['default_account_password'] ? 'default_account_password' : 'password',
                    'password_decrypted' => !empty($args['smtp_pass']),
                ));
            }
        }
        $this->debug_log('smtp_connect final', $this->debug_smtp_context($args, array(
            'source' => $source,
            'smtp_overwrite' => $this->smtp_overwrite,
            'sender' => $this->sender,
        )));
    ...
    {
        return (bool) $this->rcmail->config->get('additional_smtp_debug', false);
    }
    ...
    private function debug_log(string $message, array $context = array()): void
    {
        ...
        rcube::write_log(
            'roundcube',
            'additional_smtp: ' . $message . (!empty($parts) ? ' | ' . implode(', ', $parts) : '')
        );
    }

    private function debug_smtp_context(array $args, array $extra = array()): array
    {
        return array_merge(array(
            'smtp_server' => $args['smtp_server'] ?? null,
            'smtp_port' => $args['smtp_port'] ?? null,
            'smtp_user' => $args['smtp_user'] ?? null,
            'smtp_auth_type' => $args['smtp_auth_type'] ?? null,
            'smtp_helo_host' => $args['smtp_helo_host'] ?? null,
            'nosavesent' => isset($args['nosavesent']) ? (bool) $args['nosavesent'] : null,
            'enabled' => isset($args['enabled']) ? (bool) $args['enabled'] : null,
            'has_smtp_pass' => array_key_exists('smtp_pass', $args) ? !empty($args['smtp_pass']) : null,
        ), $extra);
    }
}
